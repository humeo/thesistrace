...
versions, and checksums. It never contains Alpha Values, Forward Return Labels,
raw orders, or fills. A mutable Tracking Head only points to one Checkpoint.
...
Execution Contract produce canonically exact Factor summaries, Strategy Daily
Observations, Strategy aggregates, and Terminal Strategy State. Intermediate
Alpha Values, Labels, orders, and fills may be regenerated and compared by an
explicit equivalence test but need not be durable results. Standard rolling
504-session ResearchRun output is not the comparator for a later continuous
DailyTrack.
_Avoid_: Persisted-intermediate requirement, tolerance-only comparison, latest
rolling Run, two calculation kernels
