        ...
        return jwt.encode(
            {
                "iss": issuer,
                "aud": audience,
                "sub": str(TEST_RESEARCHER.researcher_id),
...
def test_mounted_oauth_streamable_http_read_loop_and_fail_closed_boundaries(
    ...
    try:
        ...
        raw_subject = str(TEST_RESEARCHER.researcher_id)
        expected_subject = f"oauth_{sha256(raw_subject.encode()).hexdigest()[:32]}"
        ...
        assert raw_subject not in serialized_events
