...
def test_batch_worker_replicas_claim_distinct_fifo_batches_without_blocking_research(
    ...
    try:
        with TestClient(create_app(settings)) as client:
            ...
            research_claims = [
                event for event in research_events if event["event"] == "worker_claim"
            ]
            assert [event["resource_id"] for event in research_claims] == [ordinary["id"]]
            ...
            assert _stderr_events(idle_research)[-1] == {
