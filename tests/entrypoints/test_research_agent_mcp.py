...
def test_stdio_requires_one_explicit_researcher_identity(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    name = research_agent_mcp.RESEARCHER_ID_ENVIRONMENT
    monkeypatch.delenv(name, raising=False)
    with pytest.raises(RuntimeError, match="must be a Researcher UUID"):
        research_agent_mcp._researcher_id_from_environment()

    monkeypatch.setenv(name, "not-a-researcher")
    with pytest.raises(RuntimeError, match="must be a Researcher UUID"):
        research_agent_mcp._researcher_id_from_environment()

    monkeypatch.setenv(name, "018f6f7e-8342-7c9a-a4df-9a86147d2e01")
    assert str(research_agent_mcp._researcher_id_from_environment()) == (
...
def test_packaged_stdio_entrypoint_sanitizes_unavailable_core_context(
    ...
    environment = {
        **os.environ,
        research_agent_mcp.RESEARCHER_ID_ENVIRONMENT: (
            "018f6f7e-8342-7c9a-a4df-9a86147d2e01"
        ),
