...
test(`Chat rejects an invalid model-authored A2UI surface and remains usable: ${scenario}`, async ({ page }) => {
  ...
  await expect(safeError).toContainText("This research view could not be prepared. The conversation is still available.");
  await expect(safeError).toContainText("INVALID_LIFECYCLE");
...
test("A2UI shows real running state and supports keyboard and narrow-screen result interactions", async ({ page }, testInfo) => {
  ...
  try {
    ...
    const runId = (await runSurface.getByRole("link", { name: /^run_/ }).innerText()).trim();
    ...
    const running = page.getByRole("region", { name: `ResearchRun ${runId}`, exact: true }).last();
    ...
    expect(await page.getByRole("article", { name: "Research surface" }).count()).toBeGreaterThanOrEqual(2);
    await expect(page.getByRole("region", { name: `ResearchRun ${runId}`, exact: true }).last()).toContainText("succeeded");
    ...
    const card = result.getByRole("region", { name: `ResearchRun ${runId}`, exact: true });
    const metrics = card.locator(".chat-a2ui-metrics");
    await expect(metrics.locator("dt")).toHaveText(["1S Rank IC", "5S Rank IC", "20S Rank IC"]);
    const values = await metrics.textContent();
    const reload = card.getByRole("button", { name: "Reload view" });
    await keyboardFocus(page, reload);
    const reloaded = page.waitForResponse(response => new URL(response.url()).pathname === `/api/research-runs/${runId}`);
    await page.keyboard.press("Enter");
    expect((await reloaded).status()).toBe(200);
    await expect(metrics).toHaveText(values!);

    await page.setViewportSize({ width: 390, height: 844 });
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
    const navigation = card.getByRole("link", { name: runId, exact: true });
    for (const target of [copy, reload, navigation]) {
      const bounds = await target.boundingBox();
      expect(bounds?.width).toBeGreaterThanOrEqual(44);
      expect(bounds?.height).toBeGreaterThanOrEqual(44);
    }
...
test("admitted Research artifacts and a DailyTrack outlive the Chat that created them", async ({
  ...
  // The admitted resource card stays readable and can reload current Core state.
  ...
  await expect(page.getByRole("region", { name: `ResearchRun ${runId}`, exact: true })).toHaveCount(1);
  ...
  await expect(agentRunStatus(page)).toHaveText("Run complete", { timeout: 90_000 });
  const strategyRunId = (await page.getByRole("region", { name: /^ResearchRun run_/ }).last().getByRole("link", { name: /^run_/ }).innerText()).trim();
  ...
  const surfaceCountBeforeReplay = await page.getByRole("article", { name: "Research surface" }).count();
  expect(surfaceCountBeforeReplay).toBeGreaterThanOrEqual(5);
  await page.reload();
  await expect(page).toHaveURL(durableUrl);
  await expect(agentRunStatus(page)).toHaveText("Run complete");
  await expect(page.getByRole("article", { name: "Research surface" }))
    .toHaveCount(surfaceCountBeforeReplay);
  await expect(page.getByRole("region", {
    name: "Alpha proposal: Low-volatility factor",
  })).toContainText("Chat-owned");
  await expect(page.getByRole("region", { name: `ResearchRun ${runId}`, exact: true }).last()
    .locator(".chat-a2ui-metrics")).toContainText(cardRankIc);
...
test("the same Chat entry runs and explains a real Strategy Backtest", async ({
  ...
  await expect(await revealToolActivity(page, "submit_research_run", "complete"))
    .toBeVisible();
  const runId = (await page.getByRole("region", { name: /^ResearchRun run_/ }).last().getByRole("link", { name: /^run_/ }).innerText()).trim();
  if (runId === undefined || !/^run_[a-f0-9]{20}$/.test(runId)) {
...
test("a connected MCP Tool response disconnect becomes a durable failed Run and recovers", async ({ page }) => {
  ...
  try {
    ...
    await expect.poll(() => serviceLogs("agent", logsSince)).toContain('"event":"agent_run_finished"');
    const logs = serviceLogs("agent", logsSince);
...
function serviceLogs(service: "agent", since: string): string {
  const result = spawnSync(
    "docker",
    ["logs", "--since", since, `${testProjectName()}-${service}-1`],
    {
      encoding: "utf8",
      killSignal: "SIGKILL",
      stdio: ["ignore", "pipe", "pipe"],
  ...
  if (result.status !== 0) {
    throw new Error(`Could not read ${service} logs: ${result.error?.message ?? result.status}`);
  }
