...
RESEARCHER_ID_ENVIRONMENT = "THESISTRACE_RESEARCH_AGENT_RESEARCHER_ID"
    ...
    try:
        with open_core_runtime(settings) as runtime:
            ...
            authority = local_operator_authority(
                researcher_id=_researcher_id_from_environment(),
...
def _researcher_id_from_environment() -> UUID:
    value = os.environ.get(RESEARCHER_ID_ENVIRONMENT, "")
    try:
        return UUID(value)
    except (AttributeError, TypeError, ValueError) as error:
        raise RuntimeError(
            f"{RESEARCHER_ID_ENVIRONMENT} must be a Researcher UUID"
        ) from error
