...
def _population_std(arguments: tuple[BuiltinArgument, ...]) -> NumericSeries:
    ...
    for value in series:
        ...
        try:
            finite_variance = _finite(float(variance))
        except OverflowError:
            finite_variance = None
        result.append(None if finite_variance is None else math.sqrt(max(finite_variance, 0.0)))
    return tuple(result)
...
BUILTIN_DEFINITIONS = (
    ...
    BuiltinDefinition(
        ...
        numeric_behavior=(
            "Average ordinal ranks map to 0 through 1; ties share rank and one value is 0.5."
        ),
        work=_ONE_STEP_WORK,
        evaluator=_cross_section_only,
    ),
    BuiltinDefinition(
        identifier="lag",
