...
`apps/agent/src/bridge-log-privacy.test.ts` drives the installed public bridge through
five previously leaking paths. Agent operational diagnostics instead use the
closed `apps/agent/src/run-telemetry.ts` metadata writer. Keep these regressions when
...
`apps/agent/src/ask-user-schema.test.ts` captures actual Responses request schemas,
...
cases in `apps/agent/src/research-runtime.integration.test.ts` additionally verify
...
`apps/agent/src/compaction-candidates.test.ts` verifies these boundaries with an offline
