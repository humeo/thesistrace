...
def test_mcp_image_stdio_probe_requires_unique_order_and_clean_process_exit() -> None:
    smoke = _load_mcp_smoke_module()
    source = (
        Path(__file__).resolve().parents[3] / "tests" / "production_mcp_image_smoke.py"
    ).read_text()

    assert len(smoke.SAFE_STDIO_TOOL_ORDER) == 17
    assert len(set(smoke.SAFE_STDIO_TOOL_ORDER)) == 17
    assert "tuple(names) == SAFE_STDIO_TOOL_ORDER" in source
    assert "len(set(names)) == len(SAFE_STDIO_TOOL_ORDER)" in source
...
def test_mcp_evidence_sanitizer_atomically_redacts_shared_canaries(
    ...
    assert evidence.read_text().splitlines() == ["<redacted>"] * len(canaries.SENSITIVE_CANARIES)
...
def test_agent_report_decode_errors_fail_closed_without_private_details(
    ...
    sanitizer = _load_mcp_sanitizer_module()
    (tmp_path / "index.html").write_text(f"<script>data:application/zip;base64,{payload}</script>")
...
def _load_smoke_module() -> ModuleType:
    path = Path(__file__).resolve().parents[3] / "tests" / "production_image_smoke.py"
...
def _load_mcp_smoke_module() -> ModuleType:
    path = Path(__file__).resolve().parents[3] / "tests" / "production_mcp_image_smoke.py"
...
def _load_mcp_sanitizer_module() -> ModuleType:
    path = Path(__file__).resolve().parents[3] / "tests" / "sanitize_production_mcp_evidence.py"
    tests_root = str(path.parent)
    sys.path.insert(0, tests_root)
    try:
