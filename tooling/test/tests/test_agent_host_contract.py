...
def test_agent_telemetry_has_no_content_store_or_framework_trace_exporter() -> None:
    ...
    assert "'@ag-ui/mastra@1.1.1': tooling/patches/@ag-ui__mastra@1.1.1.patch" in workspace
    for image in ("agent", "auth", "web"):
        dockerfile = (ROOT / "apps" / image / "Dockerfile").read_text()
        assert dockerfile.index("COPY tooling/patches ./tooling/patches") < dockerfile.index(
...
def test_agent_compose_identity_receives_only_provider_and_process_configuration() -> None:
    ...
    assert "dockerfile: apps/agent/Dockerfile" in compose
