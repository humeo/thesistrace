...
class DataDependencies:
    ...
    def unavailable_families(
        ...
        return frozenset(
            ...
            or coverage[family_id].start > (
                start if family_id == "equity.industry_membership"
                and not self.industry_requires_warmup else calculation_start
            )
    ...
    def unavailable_for_sessions(
        ...
        if first_index < lookback:
            return unavailable | (self.required_families if self.industry_requires_warmup
                                  else self.required_families - {"equity.industry_membership"})
        return unavailable
    ...
    return DataDependencies(
        ...
        required_families=frozenset({
            ...
            *({"equity.industry_membership"}
              if neutralization == "industry" or require_industry else set()),
        }),
        field_ids_by_family={
            family_id: frozenset(
        ...
        industry_requires_warmup=require_industry,
    )
