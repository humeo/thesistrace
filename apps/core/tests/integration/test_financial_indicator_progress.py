...
def test_indicator_pending_survives_unmatched_collection_and_reopens(core_settings):
    ...
    try:
        ...
        assert reopened.pending(instrument) == (("2020-03-31", "2020-05-04"),)
    finally:
        with database.transaction() as tx:
            tx.execute(
...
def test_indicator_unknown_report_stays_pending_after_unrelated_reconciliation(core_settings):
    initialize_core(core_settings.database_url)
    database = PostgresDatabase(core_settings.database_url)
    database.open()
    instrument = "unknown-indicator-" + uuid4().hex
    try:
        store = FinancialIndicatorProgressStore(database)
        store.require_report(instrument, report_period=None, announced_on="2020-04-20")
        store.require_report(instrument, report_period=None, announced_on="2020-04-20")
        store.record_reconciliation(
            instrument,
            checked_through="2020-05-01",
            observed_reports=(("2020-03-31", "2020-04-21"),),
            observation_sha256="d" * 64,
        )
        assert store.pending(instrument) == ((None, "2020-04-20"),)
    finally:
        with database.transaction() as tx:
            tx.execute(
                "DELETE FROM data.financial_indicator_report_targets WHERE instrument_id=%s",
...
def test_indicator_discovery_targets_commit_atomically_with_announcement_receipt(core_settings):
    ...
    announcement = FinancialAnnouncement(
        announcement_id=unique * 2,
        category="补充更正",
        ts_code="000001.SZ",
        name="Example",
        title="更正公告",
        source_published_date="2020-04-20",
        report_period=None,
        url="https://example.test/correction",
    )
    from dataclasses import replace

    invalid = replace(announcement, announcement_id="f" * 64, ts_code="000002.SZ")
    discovery = FinancialAnnouncementDiscovery(
        start_date="2020-04-20",
        end_date="2020-04-20",
        completed_categories=FINANCIAL_ANNOUNCEMENT_CATEGORIES,
        announcements=(announcement, invalid),
        gaps=(),
    ...
    try:
        ...
        progress = FinancialIndicatorProgressStore(database)
        with pytest.raises(FinancialDailyRefreshError, match="IDENTITY_INVALID"):
        ...
        valid = replace(discovery, announcements=(announcement,))
        for _ in range(2):
            daily.record_discovery(
                idempotency_key=key,
                discovery=valid,
        ...
        assert progress.pending(instrument) == ((None, "2020-04-20"),)
    finally:
        with database.transaction() as tx:
            tx.execute(
                "DELETE FROM data.financial_indicator_report_targets WHERE instrument_id=%s",
...
def test_daily_indicator_dispatch_retries_frozen_work_and_keeps_source_failures(
    ...
    try:
        ...
        with database.transaction() as tx:
            tx.execute(
                "DELETE FROM data.financial_indicator_report_targets WHERE instrument_id LIKE %s",
                (prefix + "%",),
            )
            tx.execute(
