...
@pytest.fixture
def migration_database(core_settings, monkeypatch, rank_ic_migration_target_schemas):
    from thesistrace._postgres import SchemaDefinition
    from thesistrace.migrations import rank_ic_0001

    original_publication = (
        files("thesistrace.migrations").joinpath("0002_source_publication.sql").read_text()
    )
    monkeypatch.setattr(
        rank_ic_0001,
        ...
        tuple(
            ...
            for item in rank_ic_migration_target_schemas
        ),
    )
