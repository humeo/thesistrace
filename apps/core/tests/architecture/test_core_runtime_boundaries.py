...
def test_new_core_packages_do_not_import_old_or_hosted_runtime() -> None:
    for package in CORE_PACKAGES:
        for path in (ROOT / "apps/core/src/thesistrace" / package).rglob("*.py"):
...
def test_internal_import_graph_is_layered_and_acyclic() -> None:
    ...
    for package, allowed_dependencies in allowed.items():
        ...
        source = ROOT / "apps/core/src/thesistrace" / package
...
def test_product_modules_own_their_schema_sql_and_lifecycle_tables() -> None:
    ...
    for module, owned_schema in PRODUCT_SCHEMAS.items():
        active_strings = "\n".join(
            ...
            for path in (ROOT / "apps/core/src/thesistrace" / module).rglob("*.py")
        ...
        statements = (ROOT / "apps/core/src/thesistrace" / module / "schema.sql").read_text()
...
def test_runtime_requires_independent_batch_attempt_control_directory(
    ...
    environment = {
        ...
        "THESISTRACE_BATCH_ATTEMPT_CONTROL_DIRECTORY": str(attempt_control_directory),
    }
    ...
    assert not settings.batch_attempt_control_directory.is_relative_to(settings.data_mount)
...
def test_default_backend_commands_resolve_only_to_canonical_entrypoints() -> None:
    project = tomllib.loads((ROOT / "apps/core/pyproject.toml").read_text())
    ...
    assert scripts == {
        ...
        "thesistrace-research-agent-mcp": ("thesistrace.entrypoints.research_agent_mcp:main"),
    }


def test_core_diagnostic_entrypoint_is_postgresql_only() -> None:
    source = (ROOT / "apps/core/src/thesistrace/entrypoints/diagnose.py").read_text()
...
def test_long_running_runtime_verifies_but_does_not_initialize_schema() -> None:
    runtime_source = (ROOT / "apps/core/src/thesistrace" / "entrypoints" / "runtime.py").read_text()
    initializer_source = (
        ROOT / "apps/core/src/thesistrace" / "entrypoints" / "initialize.py"
    ).read_text()
...
def test_postgres_support_contains_mechanics_but_no_product_sql() -> None:
    postgres_source = "\n".join(
        path.read_text()
        for path in (ROOT / "apps/core/src/thesistrace" / "_postgres").rglob("*.py")
    )
    ...
    data_schema = (ROOT / "apps/core/src/thesistrace" / "data" / "schema.sql").read_text()
...
def test_alpha_tree_has_only_normalized_input_and_no_dynamic_execution() -> None:
    alpha_source = (ROOT / "apps/core/src/thesistrace" / "research_kernel" / "alpha.py").read_text()
    normalized_source = (
        ROOT / "apps/core/src/thesistrace" / "research_kernel" / "alpha_expression.py"
    ).read_text()
...
def test_data_owns_the_single_authorable_field_binding_catalog() -> None:
    data_fields = (ROOT / "apps/core/src/thesistrace" / "data" / "fields.py").read_text()
    fixture_source = (ROOT / "apps/core/src/thesistrace" / "fixture.py").read_text()
...
def test_publication_hides_physical_s3_keys_and_uses_the_standard_client() -> None:
    source = (ROOT / "apps/core/src/thesistrace" / "publication" / "service.py").read_text()
    exported = (ROOT / "apps/core/src/thesistrace" / "publication" / "__init__.py").read_text()
...
def test_publication_owns_its_sql_and_never_commits_a_caller_transaction() -> None:
    service = (ROOT / "apps/core/src/thesistrace" / "publication" / "service.py").read_text()
    schema = (ROOT / "apps/core/src/thesistrace" / "publication" / "schema.sql").read_text()
...
def test_research_run_keeps_direct_admission_behind_one_atomic_sql_seam() -> None:
    run_source = (ROOT / "apps/core/src/thesistrace" / "research_run" / "service.py").read_text()
    run_schema = (ROOT / "apps/core/src/thesistrace" / "research_run" / "schema.sql").read_text()
    ...
    assert not list((ROOT / "apps/core/src/thesistrace" / "definition").glob("*.py"))
    assert not (ROOT / "apps/core/src/thesistrace" / "definition" / "schema.sql").exists()
...
def test_start_tracking_receipts_are_owned_only_by_research_runs() -> None:
    ...
    track = (ROOT / "apps/core/src/thesistrace" / "daily_track" / "service.py").read_text()
    run = (ROOT / "apps/core/src/thesistrace" / "research_run" / "service.py").read_text()
    track_schema = (ROOT / "apps/core/src/thesistrace" / "daily_track" / "schema.sql").read_text()
    run_schema = (ROOT / "apps/core/src/thesistrace" / "research_run" / "schema.sql").read_text()
    track_models = (ROOT / "apps/core/src/thesistrace" / "daily_track" / "models.py").read_text()
    run_models = (ROOT / "apps/core/src/thesistrace" / "research_run" / "models.py").read_text()
...
def test_research_run_processor_owns_claims_and_uses_module_seams() -> None:
    ...
    failure_policy_source = (
        ROOT / "apps/core/src/thesistrace" / "research_run" / "failure_policy.py"
    ).read_text()
    run_schema = (ROOT / "apps/core/src/thesistrace" / "research_run" / "schema.sql").read_text()
    worker_source = (ROOT / "apps/core/src/thesistrace" / "entrypoints" / "worker.py").read_text()
...
def test_research_execution_child_has_one_columnar_calculation_route() -> None:
    execution_source = (
        ROOT / "apps/core/src/thesistrace" / "research_run" / "execution.py"
    ).read_text()
    transport_source = (
        ROOT / "apps/core/src/thesistrace" / "research_run" / "supervised_child.py"
    ).read_text()
    service_source = (
        ROOT / "apps/core/src/thesistrace" / "research_run" / "service.py"
    ).read_text()
    compose_source = (ROOT / "deploy" / "compose.yaml").read_text()
    test_compose_source = (ROOT / "deploy" / "compose.test-run.yaml").read_text()
    image_smoke_compose_source = (ROOT / "deploy" / "compose.image-smoke.yaml").read_text()
    ...
    assert compose_source.count("benchmark-data:/var/lib/thesistrace/benchmark-data") == 2
    assert "batch-attempt-control:/var/lib/thesistrace/.batch-attempts" in compose_source
    ...
    assert ":/var/lib/thesistrace/.batch-attempts" in test_compose_source
    ...
    assert (
        image_smoke_compose_source.count(
            "${THESISTRACE_TEST_BENCHMARK_MOUNT}:/smoke-data/benchmark-data"
        )
        == 2
    )
...
def test_tracking_execution_child_has_one_columnar_calculation_route() -> None:
    calculation_source = (
        ROOT / "apps/core/src/thesistrace" / "daily_track" / "calculation.py"
    ).read_text()
...
def test_daily_track_owns_activation_sql_and_copied_origin() -> None:
    track_source = (ROOT / "apps/core/src/thesistrace" / "daily_track" / "service.py").read_text()
    track_schema = (ROOT / "apps/core/src/thesistrace" / "daily_track" / "schema.sql").read_text()
    run_source = (ROOT / "apps/core/src/thesistrace" / "research_run" / "service.py").read_text()
    run_schema = (ROOT / "apps/core/src/thesistrace" / "research_run" / "schema.sql").read_text()
    http_source = (ROOT / "apps/core/src/thesistrace" / "entrypoints" / "http.py").read_text()
    worker_source = (ROOT / "apps/core/src/thesistrace" / "entrypoints" / "worker.py").read_text()
    ...
    assert "REFERENCES research_runs.run_ownership(researcher_id, run_id)" in track_schema
...
def test_permanent_runtime_has_only_the_mounted_current_data_path() -> None:
    ...
    data_exports = (ROOT / "apps/core/src/thesistrace" / "data" / "__init__.py").read_text()
    assert not (ROOT / "apps/core/src/thesistrace" / "data" / "service.py").exists()
...
def test_daily_track_working_cache_is_private_concrete_and_worker_local() -> None:
    package = ROOT / "apps/core/src/thesistrace" / "daily_track"
    ...
    assert not (ROOT / "apps/core/src/thesistrace" / "ports.py").exists()
    assert "/api/working-cache" not in http_source


def test_financial_refresh_composes_only_its_prevalidated_candidate() -> None:
    source = (ROOT / "apps/core/src/thesistrace" / "data" / "financial_refresh.py").read_text()
...
def test_daily_financial_refresh_composes_only_its_prevalidated_candidate() -> None:
    source = (
        ROOT / "apps/core/src/thesistrace" / "data" / "daily_financial_refresh.py"
    ).read_text()
...
def test_research_kernel_run_has_no_product_or_infrastructure_dependency() -> None:
    package = ROOT / "apps/core/src/thesistrace" / "research_kernel"
    source = "\n".join(path.read_text() for path in package.rglob("*.py"))
...
def test_legacy_definition_and_research_run_modules_are_absent() -> None:
    package = ROOT / "apps/core/src/thesistrace"
    for removed in (
        "definitions.py",
        "research_runs.py",
        "bounded_research.py",
...
def _internal_dependencies(path: Path, tree: ast.AST) -> set[str]:
    root = ROOT / "apps/core/src/thesistrace"
