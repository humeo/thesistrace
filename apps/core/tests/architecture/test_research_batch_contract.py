...
def test_batch_schema_keeps_membership_separate_and_claims_structural() -> None:
    batch_schema = (ROOT / "apps/core/src/thesistrace/research_batch/schema.sql").read_text()
    run_schema = (ROOT / "apps/core/src/thesistrace/research_run/schema.sql").read_text()
    run_service = (ROOT / "apps/core/src/thesistrace/research_run/service.py").read_text()
    batch_service = (ROOT / "apps/core/src/thesistrace/research_batch/service.py").read_text()
...
def test_factor_batch_execution_has_one_supervised_child_and_owned_publication_seam() -> None:
    batch_schema = (ROOT / "apps/core/src/thesistrace/research_batch/schema.sql").read_text()
    batch_execution = (ROOT / "apps/core/src/thesistrace/research_batch/execution.py").read_text()
    batch_child = (
        ROOT / "apps/core/src/thesistrace/entrypoints/batch_research_child.py"
    ).read_text()
    batch_service = (ROOT / "apps/core/src/thesistrace/research_batch/service.py").read_text()
    run_service = (ROOT / "apps/core/src/thesistrace/research_run/service.py").read_text()
