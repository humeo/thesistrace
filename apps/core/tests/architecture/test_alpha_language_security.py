...
def test_formula_language_never_executes_python() -> None:
    calls: list[str] = []
    for path in (ROOT / "apps/core/src/thesistrace" / "alpha_language").glob("*.py"):
...
def test_formula_language_does_not_publish_python_ast() -> None:
    public = (ROOT / "apps/core/src/thesistrace" / "alpha_language" / "__init__.py").read_text()

    assert "ast" not in public


def test_research_kernel_owns_builtin_definitions() -> None:
    assert not (ROOT / "apps/core/src/thesistrace" / "alpha_language" / "builtins.py").exists()
    kernel_builtins = (
        ROOT / "apps/core/src/thesistrace" / "research_kernel" / "alpha_builtins.py"
    ).read_text()
...
def test_alpha_matrix_has_no_recursive_evaluator_or_physical_field_map() -> None:
    ...
    plan_source = (
        ROOT / "apps/core/src/thesistrace" / "research_kernel" / "series_plan.py"
    ).read_text()
...
def test_alpha_language_has_one_parser_and_no_cutover_switches() -> None:
    language_root = ROOT / "apps/core/src/thesistrace" / "alpha_language"
