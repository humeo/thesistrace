...
test("expands a fill into exact prices and separate fees without charging slippage twice", async () => {
  ...
  try {
    ...
    expect(host.textContent).toContain("Raw Open (CNY)");
    expect(host.textContent).toContain("Simulated execution price (CNY)");
    await act(async () => host.querySelector<HTMLButtonElement>("button")!.click());
    const englishDetail = host.querySelector('[aria-label="Fill price and cost details"]')!;
    expect(englishDetail.textContent).toContain("Slippage is included in the simulated execution price");
