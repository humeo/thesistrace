...
export function costInputError(field: CostField, value: string): "slippageRange" | "costInvalid" | undefined {
  try {
    if (!/^[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?$/.test(value) || value.length > 128) throw new Error();
    const amount = new Decimal(value);
    if (!amount.isFinite() || amount.lt(0)) throw new Error();
    if (field === "slippage_bps" && amount.gte(10000)) return "slippageRange";
  } catch { return "costInvalid"; }
}
