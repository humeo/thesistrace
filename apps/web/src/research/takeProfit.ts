...
export function takeProfitIssues(tiers: TakeProfitTierDraft[]): { field: TakeProfitField; message: string }[] {
  const issues: { field: TakeProfitField; message: string }[] = [];
  tiers.forEach((tier, index) => {
    for (const key of ["profitThreshold", "cumulativeReduction"] as const) {
      ...
      if (value.length > 128 || !/^[0-9]+(?:\.[0-9]+)?$/.test(value)
        ...
        issues.push({ field: `takeProfitTiers.${index}.${key}`, message: key === "profitThreshold"
          ? "Enter a positive finite profit percentage." : "Enter a cumulative reduction greater than 0% and at most 100%." });
      } else if (index > 0 && numeric <= Number(tiers[index - 1][key]) / 100) {
        issues.push({ field: `takeProfitTiers.${index}.${key}`, message: "Each tier must be greater than the preceding tier." });
      }
    }
  });
  return issues;
