...
export function holdingSessionsInputError(value: string): "holdingSessions" | undefined {
  if (value === "" || (/^[0-9]+$/.test(value) && Number.isSafeInteger(Number(value)) && Number(value) > 0)) return;
  return "holdingSessions";
}
export function stopLossInputError(value: string): "stopLossRange" | undefined {
  if (value === "") return;
