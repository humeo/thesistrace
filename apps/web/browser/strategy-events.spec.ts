...
test("trading evidence pages and follows target, order, child and fill relationships", async ({ page }) => {
  ...
  await expect(page.getByRole("region", { name: "Current related scope" })).toContainText("Rebalance targets for 2026-08-03");
  ...
  await expect(page.getByRole("region", { name: "Fill price and cost details" })).toContainText("Simulated execution price (CNY)10.01");
  await expect(page.getByRole("region", { name: "Fill price and cost details" })).toContainText("Per-share slippage (CNY)0.01");
