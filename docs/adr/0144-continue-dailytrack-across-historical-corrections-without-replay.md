...
Checkpoint in the same Generation. Every previously published Factor Summary
Snapshot, Strategy observation and aggregate, Terminal Strategy State, and
pending Strategy decision remains unchanged. A cached pending Alpha from an
earlier close is not recalculated, and its signal executes as committed even
when the target Release now contains corrected inputs that would have produced
a different historical signal.
...
reach maturity in that Advance. A new Factor Summary Snapshot uses ADR-0148's
bounded as-operated Rolling Factor Observation Cache plus newly calculated
observations; cache recovery follows the Checkpoint chain's ordered Release
sequence and does not recalculate older observations from the corrected
Release. Strategy and Benchmark processing continue from the committed
predecessor state.
