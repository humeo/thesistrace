...
`predecessor = none`; ADR-0103 defines Generation 0's Activation root, while
ADR-0109 governs a new root for a result-changing runtime correction. A mutable
Tracking Head is only a pointer to the latest successful Checkpoint, never the
result truth.

When the target Dataset Release contains a historical correction that affects
the DailyTrack's dependency closure, ADR-0144 publishes the Checkpoint as a
correction boundary in the same Generation. Its predecessor and target Release
identify exactly where corrected inputs began affecting new results. The
Activation Checkpoint followed by each Checkpoint's predecessor and target
Release recursively defines the ordered Release sequence required to reproduce
the Track; no separate mutable Release-sequence record is authoritative.
...
the immutable Checkpoint chain, its ordered Dataset Release sequence, and
pinned research semantics. Its absence does not make a Checkpoint incomplete.
...
release-sequence equivalence failures cannot be skipped. Stopping a DailyTrack
prevents new Advances and automatic retry Attempts; it never deletes the
blocked Advance or changes already published truth.
