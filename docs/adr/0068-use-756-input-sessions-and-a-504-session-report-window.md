...
Each Adjusted Research Price uses only its same-session Source Adjustment
Factor, so adjustment neither extends the Alpha calculation window nor reads a
factor after the signal session.
