...
Every cache records the DailyTrack, Generation, immutable basis Tracking
Checkpoint identity and checksum, Definition hash, calculation-kernel version,
Numeric Execution Contract, and basis Dataset Release. A missing cache or any
basis mismatch causes that cache to be discarded and rebuilt over its bounded
windows from the Activation and Checkpoint chain, following the exact ordered
Dataset Release sequence that chain binds. Under ADR-0144, a historical
correction continues in the same Generation and uses corrected data only for
results first calculated at and after the correction boundary. Cache loss can
therefore cost bounded recomputation but cannot change or destroy result truth.
Exact batch-versus-incremental verification remains an explicit correctness
