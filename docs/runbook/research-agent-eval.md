...
configuration), 8,192 output tokens per call reserved within that window,
...
Observer/Reflector calls share these Run bounds and token accounting; the
[local lifecycle runbook](local-lifecycle.md) describes their model-derived
compression thresholds. Native compression is covered by deterministic model
replay; its semantic retention quality requires separate real-model evaluation.
