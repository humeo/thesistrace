...
Export `CLI_API_KEY` in the launching shell. The Test runtime maps it to the
Agent-only `THESISTRACE_AGENT_OPENAI_API_KEY`; it does not reuse an ambient
canonical key or a generic `OPENAI_API_KEY`. Never put the key in shell
arguments or a checked-in file.
...
16-KiB input, 65,536 estimated context tokens, 8,192 output tokens per call,
256-KiB total generated bytes, 512-KiB Tool result, no model-call count limit, 180-second Provider
...
not a completed real-model or production-load qualification.
