...
- [x] 普通 Run 与现有两类 Research Batch 同时遵守分离；兼容请求共享必要 Data/Alpha，仅实际 Factor Evaluation 请求执行 Factor，恢复仍以完整任务为单位。
- [x] 网页的输入、运行详情、结果分区以及 HTTP/MCP schema 按 kind 一致；Factor 拒绝账户参数，新的 Strategy 与 Track 不展示 IC/Factor 分区。
- [x] 当前 Strategy Result 只保存策略账户及必要 Signal 产物；读取严格按 kind 区分，不创建 Factor 占位或旧合同读取适配。
- [x] 在公开计算边界使用禁止标签计算的测试依赖，证明 Strategy Run、Strategy Batch 与 Track 能完成；独立 Factor Run 仍按现有 1/5/20 Session 口径运行。
- [x] 真实 Worker、PostgreSQL、RustFS 验证按 kind 发布与失败恢复，运行受影响网页及 MCP 成功/拒绝检查。
- [x] 所有相关调用方使用单一当前合同，不为中间状态添加旧版本执行分支；形成一次可验收的完整交付并独立提交。
...
- 2026-09-12：实现、定向验收及 Standards / Spec 修复复审完成。验证与一次未复现的 Worker 等待超时记录见 [实施计划](../plans/01-separate-research-execution.md)。六项原有真实依赖重启阶段全部通过；不宣称未运行的完整性能或全产品回归通过。
