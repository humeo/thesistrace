...
- [x] 真实依赖验证旧 Run 重试仍用其冻结 Generation，新 Run 固定新 Head，Track 新 Attempt 使用已验证当前 Head 且保留 Origin 和旧检查点。切换前已发布结果和引用的不可变字节不被改写或回收。
- [x] 制定具体且可执行的切换记录：源 Head 与最终候选、各组件目标版本、备份坐标、在途 Attempt 处理、暂停与恢复条件、失败处置和核验结果。候选后源 Head 有更新时重组并重验，不能忽略 07 之后的刷新。
- [x] 直接使用最终当前合同，不开发版本迁移、vXX 升级链或旧版本兼容分支。通过复用原始数据、构建候选和验证后切换完成扩容，保留历史结果、被引用数据及必要备份。
- [x] 按现有完成或恢复语义处理在途 Attempt，在统一部署窗口暂缓新的研究准入、刷新发布及 DailyTrack 推进；同步升级 Data、Alpha Catalog、读取准入、HTTP/MCP、Worker 和 Web 后核验 Head 与就绪状态，再恢复入口。
- [x] 遵守 main 为唯一开发与集成基准的发布顺序：验证并提交 main，再由部署分支从 main 合并、推送部署分支并更新服务器；不在部署分支直接开发、不 cherry-pick 独立实现。
- [x] 实际切换沿用原子 Head 发布和生命周期保护。切换前失败不动原 Head；切换成功后回执失败先核对已发布坐标并恢复状态，不自动回退指针。应用或发布失败按经验证的保留数据方案恢复，不清库、不改指纹绕过校验。
- [x] 恢复入口后验证一条新研究、既有研究读取和 Track 推进，并完成后续正常刷新，确认新字段及各家族覆盖仍保留。操作结果、候选/Head 身份和浏览器证据与最终集成版本对应。
- [x] 应用仅保留当前合同，不引入双版本运行、双读双写、旧 API fallback 或逐行换源。Data 页不新增来源大卡片或期间差异界面，Operator 权限边界保持。
- [x] 最终交付说明区分已实施、已测试、已发布及仍受来源覆盖限制的范围。只有验收、必要 review、提交和切换核验全部完成后本票才标 complete；不把准备好候选等同于完成生产切换。
...
前述GitHub推送等待为历史状态，用户已授权并完成发布。真实生产备份恢复、三表两列升级、数据合并、Head切换、新研究、旧结果保留及Track推进已通过；正常行情刷新已推进09-11。财务后续刷新暴露旧三表发现证据被误纳入指标构建的KeyError，已通过回归与串行双审查修复并进入main。当前剩余修复上线后的财务/行业刷新和最终收尾，本票尚不关闭。详见[生产发布记录](../issue08-production-release.md)。

新增身份漏采修复4083a2ba已通过六项真实数据库回归、main复验及Standards→Spec串行审查并部署。新普通财务刷新`field226-postcutover-financial-new-identities-4083a2ba`正在执行；服务器只读计划核验确认新增身份2个、目标79个且两个新增身份全部入选。最终候选发布及行业刷新尚未完成，不关闭剩余两项验收。

本次完整发布验证触发宿主机global OOM，Worker已单独暂停避免重复耗尽内存。原Head、候选与业务数据保留，其他服务健康。继续按生产发布记录中的内存故障修复计划定位、验证、串行审查和独立提交；08保持未完成。

内存修复 7b3ce5b6 已完成完整候选核验（1,242,613 行、峰值 688.69 MiB）、main 144 项数据与 6 项 PostgreSQL 复验及串行双审查，并上线。原财务任务恢复为第 2 次执行，心跳正常；最新采样 621.8 MiB，尚未生成组合 Head。继续等待正常财务刷新终态及行业刷新，不关闭剩余验收。详见[内存修复记录](../issue08-financial-memory-fix.md)。

原财务任务现已第 2 次执行成功，Head `cd5d70e0` 已发布指标候选 `42b600f7`：5,552 个身份、截至 2026-09-11，13 家族及 10 个非目标家族引用保留。三表仍有 4 个待处理更新，指标仍有 13 个待处理目标，真实完整覆盖边界保留。行业刷新 `field226-postcutover-industry-20260911-7b3ce5b6` 已正式受理；其终态、最终 UI 与交付记录未完成前，本票不关闭。

行业刷新首次执行成功，最终 Head `4c334073` 保留全部 13 家族；12 个非行业家族与财务刷新 Head 完全相同。最终生产 Data 页确认四块、226 字段（22 行情、204 财务）和实际部分就绪状态。正常刷新验收条目已完成；仅余交付记录的最终串行审查、提交和 tracker 关闭。

## 完成 — 2026-09-14

代码与后续修复已独立提交、集成并发布；最终交付记录提交 `260c5e2e` 已保存真实备份、切换、正常刷新、浏览器验收与串行审查结果。全部 12 项验收完成，Standards 唯一 P3 已修正复审关闭，Spec 0 项偏差。依本地 tracker 规则将本票标 complete；来源 pending 不冒充非空完整历史。
