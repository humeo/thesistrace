...
49. As a research author, I want price-based research to use causal cumulative-adjusted Research Prices, so that corporate-action adjustment remains continuous without reading a future reference factor.
...
51. As the operator, I want each Adjusted Research Price to use only its same-session Source Adjustment Factor, so that later data never rescales historical Alpha inputs.
...
- Dataset publication tests cover root and incremental Releases, predecessor and correction manifests, object reuse, atomic failure, catch-up sessions, source/canonical separation, field/schema immutability, suspension evidence, Base Pool evidence, Universe ranking, future-invariant causal adjustment, and no historical source scan on the normal daily path.
