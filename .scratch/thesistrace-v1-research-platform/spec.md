...
9. As the operator, I want every Dataset Release to expose its predecessor, appended session range, correction change-set, schemas, objects, adjustment factors, and checksums, so that its complete provenance is inspectable.
...
49. As a research author, I want price-based research to use dynamic front-adjusted Research Prices, so that corporate-action adjustment remains continuous within the selected Data Generation.
...
51. As the operator, I want each instrument's latest retained Source Adjustment Factor to be the Data Generation's front-adjustment reference, so that the latest Adjusted Research Price equals the latest Raw Market Price.
...
- Dataset publication tests cover root and incremental Releases, predecessor and correction manifests, object reuse, atomic failure, catch-up sessions, source/canonical separation, field/schema immutability, suspension evidence, Base Pool evidence, Universe ranking, dynamic front-adjustment rescaling, and no historical source scan on the normal daily path.
