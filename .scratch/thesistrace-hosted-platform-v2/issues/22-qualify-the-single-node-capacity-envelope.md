...
- [x] Non-Worker services remain within `5 GiB` and `2` CPU cores, leaving at least `2 GiB` memory and `0.5` CPU outside container budgets for the host.
...
- [x] Invitation issuance remains disabled when the measured capacity gate is missing or failing.
