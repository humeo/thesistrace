# 23 — Prove Hosted V2 Launch Qualification
...
**Blocked by:** 18 — Harden the Cloudflare and Caddy edge; 20 — Release through migrations, maintenance, and rollback; 21 — Back up and restore the coordinated platform; 22 — Qualify the single-node capacity envelope; 24 — Deliver Hosted Local Acceptance on 2C4G; `thesistrace-v1-research-platform/28 — Prove the revised V1 product chain`.

**Status:** ready-for-human

- [ ] Launch Qualification rejects Hosted Local Acceptance evidence and runs only on a qualifying runtime whose recorded CPU and memory satisfy ticket 22's complete same-node budget and host-reserve contract.
...
- On 2026-08-02 this ticket was retained as the production Launch Qualification
  rather than weakened to fit the current 2-CPU, 4-GiB local Docker runtime.
  It requires an Operator-controlled qualifying node, real release-bound
  Capacity Qualification and recovery evidence, and the complete destructive
  interruption matrix. Ticket 24 owns the non-attested local development gate.
