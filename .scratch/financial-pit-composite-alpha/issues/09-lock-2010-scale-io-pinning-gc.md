...
- [ ] A committed deterministic benchmark represents the 2010-to-current
  market and financial shape with a full ordinary A-share universe and wide
  statement columns.
- [ ] Descriptor inspection, Claim, Pin, admission, and Data Overview open no
  Parquet object.
- [ ] A price-only Formula reads zero financial Raw Financial Batches,
  statement objects, or financial partitions.
- [ ] A financial-only or mixed Formula reads only referenced families,
  projected fields, relevant session partitions, and selected instruments.
- [ ] ResearchRun and DailyTrack perform no instrument-by-session-by-field
  Python scan and no one-file-per-instrument read pattern.
- [ ] Cold and warm benchmarks record P50 and P95 duration, objects opened,
  bytes read, rows scanned, and peak memory for descriptor, price-only,
  financial-only, mixed, and Tracking Advance cases.
- [ ] Repository-owned budgets are derived from the representative benchmark
  and fail on a material regression without arbitrary sleep or public network.
- [ ] An Attempt pinned to Generation A completes unchanged while a Financial
  or Market Refresh publishes Generation B.
- [ ] Published Heads, retained Generation ownership, ResearchRun Attempts, and
...
- [ ] Objects with no remaining durable Generation or execution reference
  become collectible and are removed without affecting surviving results.
- [ ] Identical full Financial Refresh candidates demonstrate bounded object
  growth through content reuse.
- [ ] Corrupt or missing referenced objects fail closed with actionable
