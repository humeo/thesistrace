...
} from "../dist/research-eval.js";
import { readModelRegistry } from "../dist/model-registry.js";
import { RegisteredModelRuntime } from "../dist/model-runtime.js";
import { AGENT_LIMITS } from "../dist/guarded-language-model.js";
import { MAX_ACTIVE_AGENT_RUNS } from "../dist/durable-agent-runner.js";
import { MAX_CHAT_MESSAGE_BYTES } from "../dist/chat-request.js";
import { RESEARCH_EVAL_TURN_OBSERVATION_MS, observeResearchEvalStream as readEvents, requestResearchEvalTurn, researchEvalConversationMeetsOutcome, researchEvalToolRetryCount, unresolvedResearchEvalToolFailure } from "../dist/research-eval-stream.js";
import { researchEvalMemoryFactsSchema } from "../dist/research-eval-memory.js";
...
async function evaluate({ candidate, candidateBytes, repetitions, effort, registryValue, providerBaseUrl, providerOptions, phase, budget }) {
  ...
  const setupDigest = digest(Buffer.concat(await Promise.all([
    "tests/browser/prepare_current_data.py", "tests/fixtures/tushare-financial-capability.json",
    "tests/browser/research_eval_formula.py", "agent/scripts/eval-research.mjs", "pnpm-lock.yaml",
...
function formulaMatches(formula, outcome) {
  let output;
  try {
    output = execFileSync("uv", ["run", "python", "tests/browser/research_eval_formula.py"], {
