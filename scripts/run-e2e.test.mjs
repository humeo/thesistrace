...
for (const mode of ['success', 'test-failure', 'cleanup-failure']) test(`E2E groups preserve lifecycle evidence after ${mode}`, () => {
  ...
  try {
    mkdirSync(join(cwd, 'scripts')); mkdirSync(join(cwd, 'bin'));
    for (const file of ['run-e2e.mjs', 'e2e-groups.mjs']) copyFileSync(resolve('scripts', file), join(cwd, 'scripts', file));
    ...
    `);
    executable('scripts/test-runtime', `
