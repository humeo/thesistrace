...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === "/__test/seed-operator-directory") {
      if (Object.keys(body).length !== 0) {
        json(response, 400, { code: "AUTH_FIXTURE_INPUT_INVALID" });
        return;
      }
      ...
      json(response, 200, parsedObject(stdout));
      return;
    }
    if (url.pathname === "/__test/resend-emails") {
      if (
      ...
      const upstream = await fetch("http://resend-fake:8300/__test/emails", {
        method: body.method,
      });
      if (!upstream.ok) throw new Error("AUTH_FIXTURE_RESEND_UNAVAILABLE");
